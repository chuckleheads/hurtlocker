...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is /hab/svc/jobsrv/config/config.toml)")
	viper.SetDefault("port", 10001)
	viper.SetDefault("http_port", 10002)
	viper.SetDefault("datastore.host", "localhost")
	viper.SetDefault("datastore.port", 26257)
	viper.SetDefault("datastore.database", "jobsrv")
...
func initConfig() {
	if cfgFile != "" {
		...
		home := filepath.Dir("/hab/svc/jobsrv/config")
