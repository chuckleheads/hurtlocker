...
import (
	...
	pba "github.com/chuckleheads/hurtlocker/components/jobsrv/api/jobs"
	...
	pbs "github.com/chuckleheads/hurtlocker/components/jobsrv/jobs"
	srv "github.com/chuckleheads/hurtlocker/components/jobsrv/jobs/server"
...
func runGRPC(config *config.Config, lis net.Listener) {
	...
	pbs.RegisterJobsServer(server, srv.NewServer(db))
...
func runHTTP(config *config.Config, clientAddr string) {
	...
	if err := pba.RegisterJobsHandlerFromEndpoint(context.Background(), mux, clientAddr, opts); err != nil {
